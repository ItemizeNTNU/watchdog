...
`
		...
		[/input: config.server.input\(\)(?!\.replace)/, `input: { server: config.server.input().server.replace(/\\.js$/, ".ts") }`],
		[/input: config.serviceworker.input\(\)(?!\.replace)/, `input: config.serviceworker.input().replace(/\\.js$/, '.ts')`],
		// Add preprocess
...
function updateWebpackConfig() {
	...
	replaceInFile(path.join(projectRoot, 'webpack.config.js'), [
		...
		[/entry: config\.serviceworker\.entry\(\)/, `entry: { 'service-worker': config.serviceworker.entry()['service-worker'].replace(/\\.js$/, '.ts') }`],
